...
import { computeAjustesMeta, computeRateioFator, PLANOS_POR_COMPETENCIA_IDS } from '@/hooks/useMetasResultados';
...
describe('resultados operação — rateio e pró-rata', () => {
  ...
  it('todos os planos de imposto são apurados por competência', () => {
    const impostos = [
    ...
    for (const plano of impostos) expect(PLANOS_POR_COMPETENCIA_IDS.has(plano)).toBe(true);
