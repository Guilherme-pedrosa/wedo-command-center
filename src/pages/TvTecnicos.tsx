...
export default function TvTecnicos() {
  ...
  // Fetch last API sync timestamp.
  // Important: sync-all is logged in fin_sync_log, while sync-os is logged in sync_log.
  // We must compare both tables and pick the newest timestamp.
  const { data: lastSync } = useQuery({
    queryKey: ['last_sync_os'],
    queryFn: async () => {
      const [finRes, legacyRes] = await Promise.all([
        supabase
          .from('fin_sync_log')
          .select('created_at')
          .eq('tipo', 'sync-all')
      ...
      const finDate = finRes.data?.created_at ? new Date(finRes.data.created_at).getTime() : 0;
      const legacyDate = legacyRes.data?.created_at ? new Date(legacyRes.data.created_at).getTime() : 0;

      if (!finDate && !legacyDate) return null;
      return new Date(Math.max(finDate, legacyDate)).toISOString();
    },
    staleTime: 60 * 1000,
    refetchInterval: 60 * 1000,
    refetchIntervalInBackground: true,
