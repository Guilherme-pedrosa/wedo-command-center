...
export default function ComissoesVendedoresPage() {
  ...
  const query=useQuery({queryKey:['comissoes',periodo],queryFn:()=>carregarComissoes(periodo.inicio,periodo.fim,(n,total)=>setProgressoFinanceiro(`Conferindo financeiro no GC: ${n} de ${total} clientes`)),retry:1});
  ...
  return <div className="p-4 md:p-6 space-y-5">
    ...
    {query.isPending||query.isFetching?<p role="status">Carregando conferência… {progressoFinanceiro}</p>:query.isError?<div role="alert" className="text-destructive">Não foi possível carregar as comissões: {(query.error as Error).message}. <Button variant="outline" onClick={()=>void query.refetch()}>Tentar novamente</Button></div>:<>
      ...
      <div className="rounded-lg border overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/40"><tr>{['Venda / data','Vendedor / cliente','Produtos','Valor / margem','Comissão','Pagamento do cliente','Forma de pagamento','Conferência'].map(h=><th className="p-3 text-left whitespace-nowrap" key={h}>{h}</th>)}</tr></thead><tbody>{filtradas.map(r=><tr key={r.venda.id} className="border-t align-top hover:bg-muted/20">
        ...
        <td className="p-3 whitespace-nowrap"><b>{formatBRL(r.comissao)} ({r.percentual}%)</b><p>{r.pagamentos.length ? `Pago: ${formatBRL(r.pago)}` : "Pagamento não registrado"}</p><p>Saldo: {formatBRL(r.saldo)}</p></td>
        ...
        <td className="p-3 min-w-52"><Button variant="outline" size="sm" onClick={()=>setSelecionada(r.venda.id)}>Conferir</Button><p className="text-xs mt-1">{r.conferidaAtual?'Conferida':'A conferir'}</p>{r.avisos.map(x=><p key={x} className="text-xs text-amber-500 mt-1">{x}</p>)}</td>
      </tr>)}</tbody></table>{!filtradas.length&&<p className="p-8 text-center text-muted-foreground">Nenhuma venda encontrada para estes filtros.</p>}</div>
    ...
    {atual&&query.data&&<Detalhes key={atual.venda.id} dados={atual} parametros={query.data.parametros} isAdmin={isAdmin} onClose={()=>setSelecionada(null)}/>}
...
function Detalhes({dados,parametros,isAdmin,onClose}:{dados:Calculada;parametros:Parametros;isAdmin:boolean;onClose:()=>void}) {
  ...
  const sim=calcularVenda({...dados,conferencia:{venda_id:dados.venda.id,ajustes,conferido:false}},parametros);
  const [alterado,setAlterado]=useState(false);
  const atualizar=(patch:Partial<Conferencia['ajustes']>)=>{setAjustes(a=>({...a,...patch}));setAlterado(true);};
  const salvar=async()=>{if(ajustes.vendedorNome?.trim()&&!ajustes.justificativa?.trim()){toast.error('Informe a justificativa da identificação do vendedor.');return;}setSalvando(true);try{await salvarConferencia({venda_id:dados.venda.id,ajustes,conferido:true,assinatura:assinaturaVenda({...dados,conferencia:{venda_id:dados.venda.id,ajustes,conferido:true}},parametros)});await qc.invalidateQueries({queryKey:['comissoes']});setAlterado(false);toast.success('Conferência salva com histórico.');}catch(e){toast.error((e as Error).message);}finally{setSalvando(false);}};
  ...
  return <Dialog open onOpenChange={onClose}><DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Conferir venda {dados.venda.codigo}</DialogTitle></DialogHeader>
    ...
    {dados.recebimentos.map(r=><p key={r.gc_id} className="text-sm border-t pt-2">#{r.gc_codigo} · {formatBRL(parseMoney(r.valor_total ?? r.valor))} · vence {dataBR(r.data_vencimento)} · {r.liquidado?`Baixado em ${dataBR(r.data_liquidacao)}`:'Em aberto'} · {r.nome_forma_pagamento||'Forma não informada'}</p>)}
