...
export default function RecebimentosPage() {
  ...
  const filtered = useMemo(() => {
    ...
    const base = recebimentos.filter((r: any) => {
      ...
      if (pendenteBaixaGC && !(r.pago_sistema && !isGcSettled(r))) return false;
  ...
  const baixaGCBadge = (r: any) => {
    if (isGcSettled(r)) return <span className="text-emerald-500 text-[10px]">✅ Baixado</span>;
    if (r.pago_sistema && !isGcSettled(r))
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Pagination + Selection Summary */}
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            {selected.size > 0 && (
              <span className="text-foreground">
                {selected.size} selecionado{selected.size !== 1 ? "s" : ""} · <span className="font-semibold text-primary">{formatCurrency(selectedTotal)}</span>
              </span>
            )}
          </div>
          {totalPages > 1 && (
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        ...
        {/* Paginação movida para o topo (acima da tabela) */}
      </div>
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border p-4 flex items-center justify-between z-50">
          ...
          <div className="flex gap-2">
            ...
            {selectedItems.some((r: any) => r.pago_sistema && !isGcSettled(r)) && (
