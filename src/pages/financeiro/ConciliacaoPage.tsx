...
export default function ConciliacaoPage() {
  ...
  const handleSyncGC = async () => {
    ...
    try {
      ...
      const result = await syncByMonthChunks(filtros);
                    ...
                    {e.tipo === "CREDITO" ? (
                      <>
                        ...
                        {searchedLancamentos.recebimentos.map((r: any) => (
                          ...
                          >
                            <div className="font-medium text-xs break-words">{r.descricao}</div>
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              <span className="font-bold text-xs text-primary">{formatCurrency(Number(r.valor))}</span>
