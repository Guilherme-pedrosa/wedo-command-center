...
function computeValorPecasCusto(
  ...
  for (const p of produtos) {
    ...
    const itemDesconto = parseFloat(String(prod.desconto_valor || prod.desconto || "0")) || 0;
    const itemValorUnit = parseFloat(String(prod.valor_venda || prod.valor_unitario || prod.valor || "0")) || 0;
    const bruto = qtd * itemValorUnit;
    // Consignada: valor_total efetivo é zero (sem cobrança). Cobre 100% desconto OU item sem preço.
    const isConsignado = itemValorTotal <= 0.01;
    ...
    // PRIORIDADE 2: custo validado por NF (fin_produto_tributos.custo_efetivo_unit)
