...
function computeValorPecasCusto(
  ...
  for (const p of produtos) {
    ...
    if (qtd === 0) continue;
    // Peça consignada (Ecolab etc): 100% de desconto → valor_total do item = 0.
    // Não é nossa peça, não gera custo. Se houver qualquer valor faturado, conta o custo.
    const itemValorTotal = parseFloat(String(prod.valor_total || "0")) || 0;
    // Consignada / sem cobrança: valor_total efetivo do item é zero → não gera custo.
    if (itemValorTotal <= 0.01) continue;
...
async function syncVendas(
  ...
  for (const sitId of VENDAS_SITUACAO_IDS) {
    ...
    while (page <= totalPages) {
      ...
      params.tipo = sitId === "7340612" ? "vendas_balcao" : "produto";
