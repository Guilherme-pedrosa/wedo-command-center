...
serve(async (req) => {
  ...
  try {
    ...
    const { endpoint, method = "GET", payload, params, operation } = body as {
      endpoint: string;
      method?: string;
    ...
    if (operation && (operation !== "receivable_nfse" || verb !== "PUT" || !receiptId)) throw new Error("Operação de NFS-e inválida.");
    ...
    if (nfseBefore) {
      ...
      if (!response.ok || Number(result?.code ?? 200) >= 400 || ["error", "erro"].includes(result?.status)) throw new Error("GC recusou a vinculação da NFS-e.");
      const confirmed = await readFreshReceipt(receiptId!);
      assertReceivableNfseConfirmed(nfseBefore, confirmed, verifiedPayload!);
      const { error: auditError } = await admin.from("fin_sync_log").insert({ tipo: "gc_recebimento_nfse", status: "success", resposta: {
        gc_id: receiptId, nfse_numero: payload?.nfse_numero, financeiro_preservado: true,
      } });
