...
// supabase/functions/_shared/gc-user-core.ts
var GC_API_HOST = "api.gestaoclick.com";
function requireApiUserId(apiUserId) {
  const normalized = String(apiUserId || "").trim();
  if (!normalized) throw new Error("GC_API_USER_ID n\xE3o est\xE1 configurado");
  return normalized;
}
function isGestaoClickApiUrl(rawUrl) {
  try {
...
function forceGcApiUserInUrl(rawUrl, apiUserId) {
  const url = new URL(rawUrl);
  url.searchParams.set("usuario_id", requireApiUserId(apiUserId));
  return url.toString();
}
function forceGcApiUserInHeaders(headersInit, apiUserId) {
  const headers = new Headers(headersInit ?? {});
  headers.set("usuario-id", requireApiUserId(apiUserId));
  return headers;
}
function forceGcApiUserInBody(body, apiUserId) {
...
async function forceGcApiUserInRequest(input, init, apiUserId) {
  const request = new Request(input, init);
  const method = request.method.toUpperCase();
  let body;
  if (method !== "GET" && method !== "HEAD") {
    const text = await request.clone().text();
    body = forceGcApiUserInBody(text, apiUserId);
  }
...
var installed = false;
function installGcUsuarioId(apiUserId) {
  if (installed) return;
  const requiredApiUserId = requireApiUserId(apiUserId);
  installed = true;
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    try {
      ...
      return originalFetch(protectedRequest, signal ? { signal } : void 0);
    } catch {
      throw new Error(
        "Chamada ao Gest\xE3oClick bloqueada: n\xE3o foi poss\xEDvel garantir o usu\xE1rio da API GC"
      );
    }
  });
}

// supabase/functions/_shared/gc-user-id.ts
var GC_API_USER_ID = "1320473";
...
// src/lib/mcp/tools/gc-catalog-write.ts
...
var idSchema2 = z6.string().trim().regex(/^\d+$/).max(30);
var moneySchema = z6.union([z6.string().trim().min(1).max(40), z6.number().finite()]);
var optionalNumber = z6.union([z6.string().trim().min(1).max(40), z6.number().finite()]).optional();
var catalogInputSchema = {
  tipo: z6.enum(["produto", "servico"]),
  nome: z6.string().trim().min(2).max(160),
  codigo: z6.string().trim().min(1).max(80),
  valor_custo: moneySchema.optional(),
  valor_venda: moneySchema.optional(),
  codigo_barra: z6.string().trim().max(80).optional(),
  descricao: z6.string().trim().max(2e3).optional(),
  observacoes: z6.string().trim().max(2e3).optional(),
  ativo: z6.boolean().default(true),
  estoque: z6.number().finite().min(0).optional(),
  grupo_id: idSchema2.optional(),
  nome_grupo: z6.string().trim().max(120).optional(),
  ncm: z6.string().trim().max(20).optional(),
  cest: z6.string().trim().max(20).optional(),
  ...
  fornecedor_ids: z6.array(idSchema2).max(50).optional(),
  permitir_nome_duplicado: z6.boolean().default(false)
};
function handle3(ctx, options, operation) {
  return runAudited(ctx, options, operation).then(({ data, requestId }) => successResult({ ok: true, request_id: requestId, ...data })).catch((error) => errorResult(error, requestIdFrom(error)));
}
function listFromGc3(response) {
  const data = gcData(response);
  return Array.isArray(data) ? data : [];
}
...
function normalizeMoney(value, field) {
  ...
  if (cents < 0) {
    throw new McpToolError("INVALID_INPUT", `${field} n\xE3o pode ser negativo.`);
  }
...
function normalizeOptionalNumber(value, field) {
  ...
  if (!Number.isFinite(number) || number < 0) {
    throw new McpToolError("INVALID_INPUT", `${field} deve ser um n\xFAmero n\xE3o negativo.`);
  }
  return String(number);
}
function catalogPath(tipo) {
...
function normalizePayload(input) {
  ...
  if (tipo === "servico") {
    return {
      ...
      body: {
        nome: input.nome,
        codigo: input.codigo,
        ...valorVenda !== void 0 ? { valor_venda: valorVenda } : {},
        ...input.observacoes || input.descricao ? { observacoes: input.observacoes || input.descricao } : {}
      }
    };
  }
  if (input.valor_custo === void 0) {
    throw new McpToolError(
      "INVALID_INPUT",
      "Para cadastrar produto, informe valor_custo. Nome, c\xF3digo interno e custo s\xE3o obrigat\xF3rios no Gest\xE3oClick."
    );
  }
  const body = {
    nome: input.nome,
  ...
  if (input.fornecedor_ids?.length) {
    body.fornecedores = input.fornecedor_ids.map((fornecedorId) => ({
...
async function ensureCatalogIsUnique(prepared) {
  ...
  const byCode = listFromGc3(
    await gcRequest(`${path}${queryString({ [codeField]: code, limite: 20 })}`)
  ).filter((row) => normalizeComparable(row[codeField]) === normalizeComparable(code));
  if (byCode.length) {
    throw new McpToolError(
      "MULTIPLE_MATCHES",
      `J\xE1 existe ${tipo === "produto" ? "um produto" : "um servi\xE7o"} com este c\xF3digo no Gest\xE3oClick.`,
      false,
      {
  ...
  const byName = listFromGc3(
    await gcRequest(`${path}${queryString({ nome: body.nome, limite: 20 })}`)
  ).filter((row) => normalizeComparable(row.nome) === normalizeComparable(body.nome));
  if (byName.length) {
    throw new McpToolError(
      "MULTIPLE_MATCHES",
      `J\xE1 existe ${tipo === "produto" ? "um produto" : "um servi\xE7o"} com este nome. Use o cadastro existente ou permita nome duplicado conscientemente.`,
      false,
      {
...
function creationPreview2(prepared) {
  const { tipo, body } = prepared;
  return {
    tipo,
    nome: body.nome,
...
var prepararCriacaoProdutoServico = defineTool6({
  name: "preparar_criacao_produto_servico",
  title: "Preparar cria\xE7\xE3o de produto ou servi\xE7o no Gest\xE3oClick",
  description: "Valida o cadastro, bloqueia c\xF3digo ou nome duplicado e gera uma pr\xE9via. N\xE3o cria o item. Produto exige nome, c\xF3digo interno e custo; servi\xE7o exige nome e c\xF3digo. Ap\xF3s confirma\xE7\xE3o expl\xEDcita, use confirmar_criacao_produto_servico.",
  ...
  handler: async (input, ctx) => handle3(
    ...
    async (actor, requestId) => {
      ...
      return prepareAction({
        ...
        payload: prepared,
        requestId,
        preview: creationPreview2(prepared)
      });
    }
  )
});
var confirmarCriacaoProdutoServico = defineTool6({
  name: "confirmar_criacao_produto_servico",
  title: "Confirmar cria\xE7\xE3o de produto ou servi\xE7o no Gest\xE3oClick",
  description: "Cria exatamente o produto ou servi\xE7o previamente preparado. S\xF3 use depois da confirma\xE7\xE3o expl\xEDcita do usu\xE1rio na conversa.",
  inputSchema: {
    ...
    async (actor) => {
      ...
      try {
        ...
        if (!prepared?.body?.nome || !["produto", "servico"].includes(prepared.tipo)) {
          throw new McpToolError("CONFIRMATION_INVALID", "O cadastro preparado \xE9 inv\xE1lido.");
        }
        await ensureCatalogIsUnique(prepared);
        const response = await gcRequest(
        ...
        if (!created?.id) {
          throw new McpToolError(
            "GC_UNAVAILABLE",
            "O Gest\xE3oClick respondeu sem o ID do item criado. N\xE3o tente novamente automaticamente."
          );
        }
        await completeAction(input.pending_action_id, {
          user_id: actor.id,
        ...
        return {
          ...
          codigo: created.codigo_interno ?? created.codigo ?? prepared.body.codigo_interno ?? prepared.body.codigo,
          source: "gestaoclick_live"
        };
      } catch (error) {
        await failAction(input.pending_action_id, error);
...
var prepararCriacaoVenda = defineTool7({
  ...
  inputSchema: {
    ...
    data: z7.string().date(),
    previsao_entrega: z7.string().date().optional(),
    ...
    transportadora_id: z7.string().trim().regex(/^\d+$/).optional(),
    observacoes: z7.string().trim().max(2e3).optional(),
    observacoes_interna: z7.string().trim().max(2e3).optional(),
    valor_frete: z7.union([z7.string().trim().min(1).max(30), z7.number().nonnegative()]).default("0"),
    condicao_pagamento: z7.enum(["a_vista", "parcelado"]).default("a_vista"),
    forma_pagamento_id: z7.string().trim().regex(/^\d+$/).optional(),
    numero_parcelas: z7.number().int().min(1).max(120).optional(),
    intervalo_dias: z7.number().int().min(0).max(3650).optional(),
    data_primeira_parcela: z7.string().date().optional(),
    plano_contas_id: z7.string().trim().regex(/^\d+$/).optional(),
    produtos: z7.array(itemSchema).max(100).default([]),
    servicos: z7.array(itemSchema).max(100).default([])
  },
...
var confirmarCriacaoVenda = defineTool7({
...
var itemSchema2 = z8.object({
  id: z8.string().trim().regex(/^\d+$/).max(30),
  quantidade: z8.number().positive().max(1e5),
  valor_unitario: z8.union([z8.string().trim().min(1).max(30), z8.number().nonnegative()]),
  detalhes: z8.string().trim().max(500).optional()
});
...
var prepararCriacaoOrcamento = defineTool8({
  ...
  inputSchema: {
    ...
    observacoes: z8.string().trim().max(2e3).optional(),
    produtos: z8.array(itemSchema2).max(100).default([]),
    servicos: z8.array(itemSchema2).max(100).default([])
  },
  annotations: { readOnlyHint: false, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle5(
    ctx,
    {
      ...
      operationType: "prepare",
      sourceSystem: "gestaoclick",
      allowedRoles: COMMERCIAL_WRITE_ROLES3,
      parameters: input,
      targetEntity: "orcamento"
    },
    async (actor, requestId) => {
...
var confirmarCriacaoOrcamento = defineTool8({
...
var prepararCriacaoOrdemServico = defineTool8({
  ...
  inputSchema: {
    ...
    data: z8.string().date(),
    observacoes: z8.string().trim().max(2e3).optional(),
    observacoes_interna: z8.string().trim().max(2e3).optional(),
    produtos: z8.array(itemSchema2).max(100).default([]),
    servicos: z8.array(itemSchema2).max(100).default([])
  },
  annotations: { readOnlyHint: false, idempotentHint: true, openWorldHint: true },
...
var confirmarCriacaoOrdemServico = defineTool8({
...
function handle6(ctx, options, operation) {
...
var buscarClienteAuvo = defineTool9({
  ...
  inputSchema: {
    nome: z9.string().trim().min(2).max(120).optional(),
    external_id: z9.string().trim().max(80).optional(),
    somente_ativos: z9.boolean().default(true),
    pagina: z9.number().int().min(1).max(1e3).default(1),
    limite: z9.number().int().min(1).max(100).default(20)
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
    {
...
var buscarEquipamentos = defineTool9({
  ...
  inputSchema: {
    cliente_auvo_id: z9.number().int().positive().optional(),
    nome: z9.string().trim().max(120).optional(),
    identificador: z9.string().trim().max(120).optional(),
    external_id: z9.string().trim().max(120).optional(),
    somente_ativos: z9.boolean().default(true),
    pagina: z9.number().int().min(1).max(1e3).default(1),
    limite: z9.number().int().min(1).max(100).default(30)
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
...
var detalharEquipamento = defineTool9({
  ...
  inputSchema: { equipamento_auvo_id: z9.number().int().positive() },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
    {
...
var consultarTarefaAuvo = defineTool9({
  ...
  inputSchema: { tarefa_auvo_id: z9.number().int().positive() },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
    {
...
var buscarTarefasPorEquipamentoAuvo = defineTool9({
  ...
  inputSchema: {
    equipamento_auvo_id: z9.number().int().positive(),
    data_inicio: z9.string().date().optional(),
    data_fim: z9.string().date().optional(),
    status: z9.enum(["abertas", "finalizadas", "todas", "com_pendencia", "iniciadas_ou_encerradas"]).default("todas"),
    limite: z9.number().int().min(1).max(20).default(10),
    max_paginas: z9.number().int().min(1).max(20).default(10)
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
...
var listarTecnicosAuvo = defineTool9({
  ...
  inputSchema: {
    nome: z9.string().trim().max(120).optional(),
    pagina: z9.number().int().min(1).max(100).default(1),
    limite: z9.number().int().min(1).max(100).default(50)
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
...
var listarTiposTarefaAuvo = defineTool9({
  ...
  inputSchema: {
    descricao: z9.string().trim().max(120).optional(),
    pagina: z9.number().int().min(1).max(100).default(1),
    limite: z9.number().int().min(1).max(100).default(50)
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
...
var prepararCriacaoTarefaAuvo = defineTool9({
  ...
  inputSchema: {
    equipamento_auvo_id: z9.number().int().positive(),
    tecnico_auvo_id: z9.number().int().positive(),
    tipo_tarefa_auvo_id: z9.number().int().positive(),
    data: z9.string().date(),
    hora_inicio: z9.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),
    duracao_minutos: z9.number().int().min(15).max(1440).default(120),
    orientacao: z9.string().trim().min(3).max(500),
    prioridade: z9.number().int().min(1).max(3).default(1)
  },
  annotations: { readOnlyHint: false, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
...
var confirmarCriacaoTarefaAuvo = defineTool9({
  ...
  inputSchema: {
    pending_action_id: z9.string().uuid(),
    confirmation_token: z9.string().length(64)
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  handler: async (input, ctx) => handle6(
    ctx,
